 ...
 //   require("../fonction.php");
        ...
        function reco_content_based($bdd){
         /*   if(!isset($bdd)){
            ...
            $allId = "SELECT constructors.constructorId FROM constructors"; $rep = $bdd -> query($allId); $allIds = $rep -> fetchAll();

            // récupération des pilotes likés
            if(isset($_SESSION["utilisateur"])){
                $recup = "SELECT liker_constructor.constructorId FROM liker_constructor WHERE liker_constructor.email_adress=".$_SESSION["utilisateur"]["email_adress"];
                $rep = $bdd -> query($recup); $ids = $rep -> fetchAll();
            }
            else{
                $recup = "SELECT liker_constructor.constructorId FROM liker_constructor WHERE liker_constructor.email_adress='a'";
                $rep = $bdd -> query($recup); $ids = $rep -> fetchAll();
            }
            ...
            for($i=0; $i<count($allIds); $i++){
                $q1 = "SELECT constructors.nationality FROM constructors WHERE constructors.constructorId=".$allIds[$i][0]; $q2 = "SELECT COUNT(results.raceId) FROM results WHERE results.constructorId=".$allIds[$i][0];
                $q3 = "SELECT COUNT(results.raceId) FROM results WHERE results.position=1 AND results.constructorId=".$allIds[$i][0];
                $q4 = "SELECT COUNT(results.raceId) FROM results WHERE (results.position=1 OR results.position=2 OR results.position=3) AND results.constructorId=".$allIds[$i][0];
            ...
            // déterminer si pilote aimé ou pas : 
                if(isset($ids)){
                    ...
                    for($j=0; $j<count($ids) && !$find; $j++){
                        if($ids[$j]["constructorId"]==$allIds[$i][0]){
                            $find=TRUE;
                        }
                    }
                    if($find){
        ...
        function reco_user_based($bdd){
           /* if(!isset($bdd)){
            ...
            $ownAdress="";
            if(isset($_SESSION["utilisateur"])){
                $q = "SELECT liker_constructor.constructorId FROM liker_constructor WHERE liker_constructor.email_adress=".$_SESSION["utilisateur"]["email_adress"];
                $ownAdress=$_SESSION["utilisateur"]["email_adress"];
            }
            else{
                $q = "SELECT liker_constructor.constructorId FROM liker_constructor WHERE liker_constructor.email_adress='a'";
                $ownAdress="a";
            }
            ...
            for($i=0; $i<count($ids); $i++){
                ...
                $userq = "SELECT liker_constructor.email_adress FROM liker_constructor WHERE liker_constructor.constructorId=".$ids[$i]["constructorId"]." AND liker_constructor.email_adress!='".$ownAdress."'";
            ...
            for($i=0; $i<count($mostLikely); $i++){
                $likelyq = "SELECT liker_constructor.constructorId FROM liker_constructor WHERE liker_constructor.email_adress='".$mostLikely[$i]["id"]."' AND liker_constructor.constructorId NOT IN (".$q.")";
                ...
                for($j=0; $j<count($nonliked); $j++){
                    ...
                    for($k=0; !$alreadyIn && $k<count($recommandation); $k++){
                        if($recommandation[$k]["id"]==$nonliked[$j]["constructorId"]){
                            $alreadyIn=TRUE;
                            $index=$k;
                        }
                    }
                    if(!$alreadyIn){
                        array_push($recommandation, array("id" => $nonliked[$j]["constructorId"], "count" => 1));
                    }
                    else{
    ...
    /*    if(!isset($bdd)){
      ...
      //  generateLike(11, 100, $bdd);
        $top8 = reco_content_based($bdd);

        for($i=0; $i<count($top8); $i++){
            $q = "SELECT * FROM constructors WHERE constructors.constructorId=".$top8[$i]["id"];
            $rep = $bdd -> query($q); $pilote = $rep -> fetch();
            echo "Recommandation ".($i+1)." : ".$pilote["name"]." <br>";
        }  

        echo "<br>  <br>";

       //reset_like();
        $top8 = reco_user_based($bdd);
        ...
        function generateLike($nbUsers, $nbLikes, $bdd){
           /* if(!isset($bdd)){
            ...
            $q = "SELECT constructors.constructorId FROM constructors";
            ...
            while($i<$nbLikes){
                ...
                $verifq = "SELECT COUNT(*) FROM liker_constructor WHERE liker_constructor.email_adress='".$users[$randomUser]."' AND liker_constructor.constructorId=".$pilotesId[$randomPilote]["constructorId"];
                $rep = $bdd -> query($verifq); $ans = $rep -> fetch();
                if($ans[0]==0){ 
                    $insertq = "INSERT INTO liker_constructor VALUES (".$pilotesId[$randomPilote]["constructorId"].", '".$users[$randomUser]."', '2000-01-01')";
        ...
        function reset_like(){
          /*  if(!isset($bdd)){
            ...
            $q = "SELECT DISTINCT liker_constructor.email_adress FROM liker_constructor WHERE liker_constructor.email_adress!='a'";
            $rep = $bdd -> query($q); $ans = $rep -> fetchAll();
            for($i=0; $i<count($ans); $i++){
                $deleteq = "DELETE FROM liker_constructor WHERE liker_constructor.email_adress='".$ans[$i]["email_adress"]."'";
