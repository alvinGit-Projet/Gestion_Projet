...
if(isset($_SERVER["HTTP_X_REQUESTED_WITH"]) && strtoupper($_SERVER["HTTP_X_REQUESTED_WITH"])=="XMLHTTPREQUEST"){
        ...
        $q = "SELECT COUNT(*) FROM liker_circuit WHERE liker_circuit.email_adress='".$mail."' AND liker_circuit.circuitId=".$id;
        ...
        if($ans[0]>0){
         /*   $statement = $bdd -> prepare($delq);
            ...
            $query = "DELETE FROM liker_circuit WHERE liker_circuit.circuitId=".$id." AND liker_circuit.email_adress='".$mail."'";
            $rep = $bdd -> query($query);
          ...
          /*  $statement = $bdd -> prepare($insq);
            $statement -> bindParam(":email", $mail); $statement -> bindParam(":id", $id);
            $rep = $statement -> execute();   */
            $query = "INSERT INTO liker_circuit VALUE(".$id.", '".$mail."', '2000-01-01')";
            $rep = $bdd -> query($query);
