    ...
    <div class="container-fluid" id="first" >
        ...
        <p>
          ...
          if(isset($infos["description"]) && $infos["description"]!=""){
              ...
              echo $descr;
              $q = 'UPDATE circuits SET circuits.description="'.$descr.'" WHERE circuits.circuitId='.$infos["circuitId"];
              $rep = $bdd -> query($q);
          }
?>
  ...
  <script>
           loged = <?php if(isset($_SESSION["utilisateur"])){
            $q = "SELECT COUNT(*) FROM liker_circuit WHERE liker_circuit.email_adress='".$_SESSION["utilisateur"]["email_adress"]."' AND liker_circuit.circuitId=".$infos["circuitId"];
          ...
          let id = <?php echo $infos["circuitId"]; ?>;
          if(loged){
            $("#like").click(function(){
              like(email, id, liked);
              if(liked){
            ...
            $("#like").click(function(){
              let bool = confirm("Vous devez être connecté pour aimer un circuit, souhaitez vous être redirigé vers une page de connexion?");
              if(bool){
                window.location.href="../utilisateur/connexion.php";
              }
            })
