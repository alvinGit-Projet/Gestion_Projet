...
if(isset($_SERVER["HTTP_X_REQUESTED_WITH"]) && strtoupper($_SERVER["HTTP_X_REQUESTED_WITH"])=="XMLHTTPREQUEST"){
        ...
        $q = "SELECT COUNT(*) FROM liker_gp WHERE liker_gp.email_adress='".$mail."' AND liker_gp.raceId=".$id;
        ...
        if($ans[0]>0){
         /*   $statement = $bdd -> prepare($delq);
            ...
            $query = "DELETE FROM liker_gp WHERE liker_gp.raceId=".$id." AND liker_gp.email_adress='".$mail."'";
            $rep = $bdd -> query($query);
          ...
          /*  $statement = $bdd -> prepare($insq);
            $statement -> bindParam(":email", $mail); $statement -> bindParam(":id", $id);
            $rep = $statement -> execute();   */
            $query = "INSERT INTO liker_gp VALUE('".$mail."',".$id.", '2000-01-01')";
            $rep = $bdd -> query($query);
