    ...
    <div class="container-fluid" id="first" >
        ...
        <div class="col-lg-6" id="courses">
            ...
            if(isset($infos["description"]) && $infos["description"]!=""){
                ...
                echo $descr;
                $q = 'UPDATE races SET races.description="'.$descr.'" WHERE races.raceId='.$infos["raceId"];
                $rep = $bdd -> query($q);
            }
...
?>
    ...
    <script>
          loged = <?php if(isset($_SESSION["utilisateur"])){
            $q = "SELECT COUNT(*) FROM liker_gp WHERE liker_gp.email_adress='".$_SESSION["utilisateur"]["email_adress"]."' AND liker_gp.raceId=".$infos["raceId"];
          ...
          if(loged){
            ...
            $("#like").click(function(){
              let bool = confirm("Vous devez être connecté pour aimer un grand prix, souhaitez vous être redirigé vers une page de connexion?");
              if(bool){
                window.location.href="../utilisateur/connexion.php";
              }
            })
